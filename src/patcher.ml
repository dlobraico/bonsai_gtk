...
(* The walk lives here; its two supporting halves are [Patcher_checks] (the structural
   checks and the path-prefix helpers) and [Patcher_fixups] (the pass context, the
   stack-name registry and the fixup queue). [patcher.mli] stays the single public
   interface for all three: the re-exports below are exactly what it always declared, and
   the split modules are internal. *)
...
let child_op = Patcher_checks.child_op
...
let rec mount ctx ~path ~is_root ~parent_kind (node : Node.t) : live =
  Patcher_checks.check_placement ~path ~is_root ~parent_kind node;
...
and mount_list
  ...
  : live list
  =
  Patcher_checks.check_unique_keys ~path cs;
...
(* The two entry points, each wrapping the walk in the pass-level bookkeeping the walk
   ...
   names the walk collected are applied once it is over (see
...
let mount ctx ~path ~is_root node =
  ...
  (match Patcher_fixups.apply_stack_claims ctx with
   | () -> ()
   | exception exn ->
     let backtrace = Stdlib.Printexc.get_raw_backtrace () in
     destroy ctx live;
...
let rec reassert_only ctx ~path (live : live) =
  ...
  Patcher_fixups.enqueue_fixups
    ctx
    ~path
    ~widget:live.widget
    ~interest:(Patcher_fixups.interest_of_kind live.node.kind);
